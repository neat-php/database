...
class QueryTraitTest extends TestCase
{
    ...
    public function provideQuery(): array
    {
        return array_merge(...array_map(function (string $method) {
            return [
                [(new Query($connection = $this->connection()))->select('*')->from('user'), $connection, $method],
                [new SQLQuery($connection = $this->connection(), "SELECT * FROM `user`"), $connection, $method],
            ];
        }, ['query', 'fetch', 'execute']));
    }
